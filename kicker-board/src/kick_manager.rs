...
const MAX_KICK_DURATION_US: f32 = 10000.0;  // 10ms (10k us) max power kick
...
impl<'a> KickManager<'a> {
    ...
    pub async fn command(&mut self, battery_voltage: f32, rail_voltage:f32, charge: bool, kick_type: KickType, kick_strength: f32) -> Result<(), ()> {
        ...
        // bound kick strength
        let kick_strength = fmaxf(fminf(kick_strength, 1.0), 0.0);
        ...
        match kick_type {
            KickType::Kick => {
                ...
                Timer::after(Duration::from_micros((MAX_KICK_DURATION_US * kick_strength) as u64)).await;
            ...
            KickType::Chip => {
                ...
                Timer::after(Duration::from_micros((MAX_CHIP_DURATION_US * kick_strength) as u64)).await;
