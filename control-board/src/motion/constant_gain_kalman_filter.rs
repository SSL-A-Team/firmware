...
pub struct CgKalmanFilter<
    'a,
    const NUM_STATES: usize,
    const NUM_CONTROL_INPUTS: usize,
    const NUM_OBSERVATIONS: usize,
> {
    ...
    > CgKalmanFilter<'a, NUM_STATES, NUM_CONTROL_INPUTS, NUM_OBSERVATIONS>
{
    ...
    pub fn predict(&mut self, u: &SMatrix<f32, NUM_CONTROL_INPUTS, 1>) {
        ...
        self.pred_estimate_cov =
            self.state_transition * self.estimate_cov * self.state_transition.transpose()
                + self.process_cov;
    }
