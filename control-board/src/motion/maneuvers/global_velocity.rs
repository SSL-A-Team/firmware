...
use ateam_common_packets::{BodyControlCommand, BodyControlManeuverExtendedTelemetry, ExtendedGlobalVelocityTelemetry};
...
impl MotionManeuver for GlobalVelocityManeuver {
    ...
    fn update(
        ...
        let BodyControlCommand::GlobalVelocity(c) = cmd else {
        ...
        let telem = BodyControlManeuverExtendedTelemetry::GlobalVelocity(ExtendedGlobalVelocityTelemetry {
            cmd_echo: c,
        });
        Ok((setpoints, telem))
    }
