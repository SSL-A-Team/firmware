...
use ateam_lib_stm32::filter::{Filter, WindowAvergingFilter};
use ateam_lib_stm32::{
...
async fn main(main_spawner: embassy_executor::Spawner) {
    ...
    let mut waf: WindowAvergingFilter<100, true, f32> = WindowAvergingFilter::new();

    let mut mv_cmd_counting_up = true;
    let mut mv_cmd = 0.0;

    const ACC_WIN: usize = 1000;

    let mut acc_buf: [f32; ACC_WIN] = [0.0; ACC_WIN];

    let mut prev_rads = bl_ccm.read_rads();
    ...
    loop {
        ...
        let got_a_packet = bl_ccm.process_packets();
        // br_ccm.process_packets();
        // fr_ccm.process_packets();

        let v = bl_ccm.read_vbus_voltage();
        let i_sp = bl_ccm.read_current_setpoint_ma();
        let i_ref = bl_ccm.read_current_estimate_ma();
        let w = bl_ccm.read_rads();
        let Vm = bl_ccm.read_vmotor_voltage_mv();

        let cur_seq_num = bl_ccm.get_latest_state_seqnum();
        if cur_seq_num != last_seq_num {
            torque_data_pub.publish_immediate(bl_ccm.get_latest_state());
        ...
        if ctr > 19 {
            // defmt::info!(
            //     "motion control type: {}",
            //     bl_ccm.get_latest_state().motion_control_type
            // );
            // defmt::info!(
        ...
        if got_a_packet && curr_ctr < ACC_WIN {
            let cur_rads = bl_ccm.read_rads();
            waf.add_sample(cur_rads);
            waf.update();

            let filt_rads = waf.filtered_value().unwrap();

            let acc = (filt_rads - prev_rads) / 0.001;
            
            prev_rads = filt_rads;

            // defmt::info!("ang acc: {}, vel: {}", acc, filt_rads);

            acc_buf[curr_ctr] = acc;

            curr_ctr += 1;
        }

        const MOTOR_I_SP: i16 = 1690;

        if curr_ctr >= ACC_WIN {
            bl_ccm.set_current_setpoint(0);

            let avg_acc = acc_buf.iter().sum::<f32>() / acc_buf.len() as f32;

            let avg_torque = 1.109e-4 * avg_acc; 

            let Km = avg_torque / (MOTOR_I_SP as f32 / 1000.0);
            let err = Km / 0.0335;

            defmt::info!("I: {}, rads: {}, acc {}, tau: {}, Km (Nm/A): {}, err: {}", MOTOR_I_SP, waf.filtered_value(), avg_acc, avg_torque, Km, err);
        } else {
            bl_ccm.set_current_setpoint(MOTOR_I_SP);
        }
