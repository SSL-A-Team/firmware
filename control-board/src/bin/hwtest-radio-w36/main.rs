...
use ateam_control_board::{
    create_dotstar_task, create_io_task, create_radio_nora_task, get_system_config,
    pins::{CommandsPubSub, LedCommandPubSub, TelemetryPubSub},
...
async fn main(main_spawner: embassy_executor::Spawner) {
    ...
    defmt::info!("radio nora task started");

    loop {
        match select::select(
            control_command_subscriber.next_message(),
            Timer::after_millis(1000),
        )
        .await
        {
            Either::First(gyro_data) => match gyro_data {
