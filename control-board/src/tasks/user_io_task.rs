...
use crate::{
    adc_v_to_battery_voltage, pins::*, BATTERY_MAX_VOLTAGE, BATTERY_MIN_CRIT_VOLTAGE,
    BATTERY_MIN_SAFE_VOLTAGE,
};
...
macro_rules! create_io_task {
    ($spawner:ident, $robot_state:ident, $p:ident) => {
        ateam_control_board::tasks::user_io_task::start_io_task(
            &$spawner,
            $robot_state,
            $p.ADC1,
            $p.PA0,
...
async fn user_io_task_entry(
    ...
    const BATTERY_VOLTAGE_FILTER_ALPHA: f32 =
        IO_TASK_LOOP_RATE_DURATION.as_millis() as f32 / BATTERY_FILTER_WINDOW.as_millis() as f32;
    ...
    loop {
        ...
        if send_extended_telem != cur_robot_state.radio_send_extended_telem {
            ...
            defmt::info!(
                "updated robot send extended telem {} -> {}",
                cur_robot_state.radio_send_extended_telem,
                send_extended_telem
            );
        }
        ...
        let batt_res_div_v =
            battery_volt_adc.read_volt_raw_f32(vref_int_read_mv as f32, vref_int_cal) * 0.9090;
        ...
        let battery_voltage_ave = battery_voltage_filter
            .filtered_value()
            .expect("iir never return None here");
        ...
        robot_state.set_battery_crit(
            battery_voltage_ave < BATTERY_MIN_CRIT_VOLTAGE
        ...
        if !robot_state.hw_init_state_valid() {
            defmt::info!(
                "loaded robot state: robot id: {}, team: {}",
                robot_id,
                robot_team_isblue
            );
