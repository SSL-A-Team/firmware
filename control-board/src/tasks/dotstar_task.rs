...
macro_rules! create_dotstar_task {
    ($spawner:ident, $led_command_subscriber:ident, $p:ident) => {
        ateam_control_board::tasks::dotstar_task::start_dotstar_task(
            &$spawner,
    ...
    loop {
        ...
        match led_command {
            ...
            ControlBoardLedCommand::Imu(imu_status_led_command) => match imu_status_led_command {
                ...
                ImuStatusLedCommand::Error => {
                    dotstars.set_color(RED, ControlDotstarIndex::Imu.into())
                }
            },
            ControlBoardLedCommand::Radio(radio_status_led_command) => {
                match radio_status_led_command {
                    RadioStatusLedCommand::Off => {
                        dotstars.set_color(BLACK, ControlDotstarIndex::Radio.into())
                    }
                    RadioStatusLedCommand::ConnectedPhys => {
                        dotstars.set_color(CYAN, ControlDotstarIndex::Radio.into())
                    }
                    RadioStatusLedCommand::ConnectedUart => {
                        dotstars.set_color(BLUE, ControlDotstarIndex::Radio.into())
                    }
                    RadioStatusLedCommand::ConnectedNetwork => {
                        dotstars.set_color(VIOLET, ControlDotstarIndex::Radio.into())
                    }
                    RadioStatusLedCommand::ConnectedSoftware => {
                        dotstars.set_color(GREEN, ControlDotstarIndex::Radio.into())
                    }
                    RadioStatusLedCommand::Ok => {
                        dotstars.set_color(GREEN, ControlDotstarIndex::Radio.into())
                    }
                    RadioStatusLedCommand::Error => {
                        dotstars.set_color(RED, ControlDotstarIndex::Radio.into())
                    }
                }
            }
            ControlBoardLedCommand::Kicker(kicker_status_led_command) => {
                match kicker_status_led_command {
                    KickerStatusLedCommand::Ok => {
                        dotstars.set_color(GREEN, ControlDotstarIndex::Kicker.into())
                    }
                    KickerStatusLedCommand::Error => {
                        dotstars.set_color(RED, ControlDotstarIndex::Kicker.into())
                    }
                }
            }
            ControlBoardLedCommand::General(control_general_led_command) => {
                match control_general_led_command {
                    ControlGeneralLedCommand::ShutdownRequested => {
                        dotstars.set_color(ORANGE_RED, ControlDotstarIndex::User2.into())
                    }
                    ControlGeneralLedCommand::BallDetected => {
                        dotstars.set_color(BLUE, ControlDotstarIndex::User2.into())
                    }
                    ControlGeneralLedCommand::BallNotDetected => {
                        dotstars.set_color(BLACK, ControlDotstarIndex::User2.into())
                    }
                    ControlGeneralLedCommand::Ok => {
                        dotstars.set_color(GREEN, ControlDotstarIndex::User1.into())
                    }
                    ControlGeneralLedCommand::Warn => {
                        dotstars.set_color(YELLOW, ControlDotstarIndex::User1.into())
                    }
                    ControlGeneralLedCommand::Error => {
                        dotstars.set_color(RED, ControlDotstarIndex::User1.into())
                    }
                }
            }
        }
...
pub async fn start_dotstar_task(
    ...
    let dotstars = Apa102::<NUM_LEDS>::new_from_pins(
        dotstar_peri,
        dotstar_sck_pin,
        dotstar_mosi_pin,
        dotstar_tx_dma,
