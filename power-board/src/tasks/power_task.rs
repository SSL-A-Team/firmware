...
use ateam_lib_stm32::{
    ...
    drivers::adc::AdcConverter,
    filter::WindowAvergingFilter,
    math::range::Range,
    power::{
        battery::LipoModel,
        model::lipo_model::{lipo_pct_interp, LIPO_6S_VOLTAGE_PERCENT},
        PowerRail,
    },
};
...
use crate::{
    config::{
        LIPO6S_BALANCE_RAW_SAMPLES_TO_VOLTAGES, LIPO_BATTERY_CONFIG_6S, POWER_RAIL_12V0_PARAMETERS,
        POWER_RAIL_3V3_PARAMETERS, POWER_RAIL_5V0_PARAMETERS, POWER_RAIL_BATTERY_PARAMETERS,
    },
    pins::*,
...
async fn power_task_entry(
    ...
    let mut power_rail_battery = PowerRail::new(
        POWER_RAIL_BATTERY_PARAMETERS,
        WindowAvergingFilter::<POWER_RAIL_FILTER_WINDOW_SIZE, true, _>::new(),
        Range::new(0.0, 2062.0),
        Range::new(0.0, 25.2),
    );

    let mut power_rail_12v0 = PowerRail::new(
        POWER_RAIL_12V0_PARAMETERS,
        WindowAvergingFilter::<POWER_RAIL_FILTER_WINDOW_SIZE, true, _>::new(),
        Range::new(0.0, 1741.0),
        Range::new(0.0, 12.0),
    );

    let mut power_rail_5v0 = PowerRail::new(
        POWER_RAIL_5V0_PARAMETERS,
        WindowAvergingFilter::<POWER_RAIL_FILTER_WINDOW_SIZE, true, _>::new(),
        Range::new(0.0, 1745.0),
        Range::new(0.0, 5.0),
    );

    let mut power_rail_3v3 = PowerRail::new(
        POWER_RAIL_3V3_PARAMETERS,
        WindowAvergingFilter::<POWER_RAIL_FILTER_WINDOW_SIZE, true, _>::new(),
    ...
    loop {
        ...
        adc_converter
            .raw_samples_to_mv(&power_rail_adc_raw_samples, &mut power_rail_voltage_samples);
        ...
        defmt::info!(
            "power rail voltages: Battery {}, 12v0 {}, 5v0 {}, 3v3 {}",
            power_rail_battery.get_rail_voltage(),
            power_rail_12v0.get_rail_voltage(),
            power_rail_5v0.get_rail_voltage(),
            power_rail_3v3.get_rail_voltage()
        );
        ...
        shared_power_state
            .set_power_rail_3v3_ok(power_rail_3v3.power_ok())
            .await;
        shared_power_state
            .set_power_rail_5v0_ok(power_rail_5v0.power_ok())
            .await;
        shared_power_state
            .set_power_rail_12v0_ok(power_rail_12v0.power_ok())
            .await;
        shared_power_state
            .set_power_ok(
        ...
        if last_battery_cell_read_time.elapsed() > BATTERY_CELL_READ_INTERVAL {
            let battery_cell_read_seq = [
                ...
                (
                    &mut power_rail_vbatt_before_lsw_adc_pin,
            ...
            adc_converter.raw_samples_to_v(
                &battery_cell_adc_raw_samples,
                &mut battery_cell_voltage_samples,
            );
            ...
            defmt::info!(
                ...
                lipo6s_battery_model.get_cell_voltages()
            );
            defmt::info!(
                "battery cell percentages {}",
                lipo6s_battery_model.get_cell_percentages()
            );
            defmt::info!(
                "battery worst cell imblanace {}",
                lipo6s_battery_model.get_worst_cell_imbalance()
            );

            if lipo6s_battery_model
                .get_cell_percentages()
                .into_iter()
                .all(|v| *v <= 0)
            {
                if cur_power_state.balance_connected {
                    ...
                    // play song
                    audio_publisher
                        .publish(AudioCommand::PlaySong(SongId::BalanceDisconnected))
                        .await;
                }
                ...
                if !cur_power_state.balance_connected {
                    ...
                    // play song
                    audio_publisher
                        .publish(AudioCommand::PlaySong(SongId::BalanceConnected))
                        .await;
                }
            ...
            if f32::abs(
                battery_cell_voltage_samples[7] - power_rail_voltage_samples[3] as f32 / 1000.0,
            ) > 1.0
            {
            ...
            if power_rail_battery.warning_flagged()
                || (cur_power_state.balance_connected && lipo6s_battery_model.battery_warn())
            {
                if !cur_power_state.battery_low_warn {
                    ...
                    // play song
                    audio_publisher
                        .publish(AudioCommand::PlaySong(SongId::BatteryLow))
                        .await;
                }
            ...
            if power_rail_battery.critical_warning_flagged()
                || (cur_power_state.balance_connected && lipo6s_battery_model.battery_crit())
            {
                if !cur_power_state.battery_low_crit {
                    ...
                    // play song
                    audio_publisher
                        .publish(AudioCommand::PlaySong(SongId::BatteryCritical))
                        .await;
                }
            ...
            shared_power_state
                .set_high_current_operations_allowed(high_current_ops_allowed)
                .await;
        }

        battery_telem_packet.set_battery_ok(lipo6s_battery_model.battery_ok() as u16);
        battery_telem_packet
            .set_battery_balance_connected(cur_power_state.balance_connected as u16);
        battery_telem_packet.set_battery_low(power_rail_battery.warning_flagged() as u16);
        battery_telem_packet
            .set_battery_critical(power_rail_battery.critical_warning_flagged() as u16);
        ...
        battery_telem_packet.set_battery_cell_imbalance_warn(
            lipo6s_battery_model.battery_cell_imbalance_warn() as u16,
        );

        battery_telem_packet.battery_mv =
            (power_rail_battery.get_rail_voltage().unwrap_or(0.0) / 1000.0) as u16;
        battery_telem_packet.battery_pct = lipo_pct_interp(
            power_rail_battery.get_rail_voltage().unwrap_or(0.0),
            &LIPO_6S_VOLTAGE_PERCENT,
        ) as u8;
...
pub async fn start_power_task(
    ...
    spawner
        .spawn(power_task_entry(
            ...
            cell5_adc_pin,
            cell6_adc_pin,
            power_rail_12v0_adc_pin,
            power_rail_5v0_adc_pin,
