...
fn write_pitches_to_file(pitch_set: Vec<(String, u16)>) {
    ...
    for pitch in pitch_set {
        ...
        generated_file_contents.push_str(
            format!(
                "pub const {}: u16 = {};{}",
                pitch.0.to_uppercase(),
                pitch.1,
                LINE_ENDING
            )
            .as_str(),
    ...
    let write_contents: bool = if !pitches_file.exists() {
        ...
        let file_contents =
            std::fs::read_to_string(pitches_file).expect("failed to open existing pitches file");

        file_contents != generated_file_contents
    };

    if write_contents {
        std::fs::write(pitches_file, generated_file_contents)
            .expect("failed to write pitches file contents");
    }
}
